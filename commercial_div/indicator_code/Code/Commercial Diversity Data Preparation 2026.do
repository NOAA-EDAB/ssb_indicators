...
Geret DePiper
2019 SOE
PPI is WPU0223 using R to download*/

cd 
clear
set fredkey "2a17d25886f2a35c25fe7e8c0d645fde"
import fred WPU0223

drop if year(daten) < 1996
rename WPU0223 value
*Deflator changed to January 2018, to be consistent with other SOE revenue representations
gen deflator = 299.4/value 
gen month = month(daten)
gen year = year(daten)
drop datestr daten
save "F:\ESRs\ESR2026\Data\diversitypplong", replace
...
#delimit ;
cd "M:\READ-SSB-Lee-spacepanels\data_folder\main";

use "veslog_species_huge_2025_08_22.dta", clear;

#delimit ;
merge m:1 tripid using "just_one_area_2025_08_22.dta", keep(1 3);
rename _merge _merge1;

#delimit ;
merge m:1 tripid using "just_one_EPU_2025_08_22.dta", keep(1 3);
rename _merge _merge2;

#delimit ;
merge m:1 tripid myspp using "areas_tripids_species_2025_08_22.dta", keep(1 3);
rename _merge _merge3;

#delimit ;
merge m:1 tripid myspp using "EPUs_tripids_species_2025_08_22.dta", keep(1 3 4 5) update;
...
drop _merge*;
merge m:1 month year using "F:\ESRs\ESR2026\Data\diversitypplong";
	gen real_revenue = nrevenue*deflator;

drop if real_revenue == .;
drop if qtykept == 0;
	save "F:\ESRs\ESR2026\Data\SOE_diversityrevenuedata_2026", replace;
