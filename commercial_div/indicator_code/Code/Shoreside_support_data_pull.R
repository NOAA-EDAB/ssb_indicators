...
#Defining states within the Mid-Atlantic
 ...
 # For all industry codes and titles see:
 # http://data.bls.gov/cew/doc/titles/industry/industry_titles.htm
 Industries <- c("311710","424460","445220","424490")
 Years <- c(2017:2021)
 for (y in Years) {
   for (x in Industries) {
 ...
 Industries <- c("424490","311710","424460","445250")
 Years <- c(2022:2023)
 for (y in Years) {
   for (x in Industries) {
     TEMP <- qcewGetIndustryData(y,"A",x) %>% 
...
for (y in 2017:2021) {
  TEMP <- getCensus(
    ...
    region = "state:10,24,36,34,42,37,51",
    key="1cd45e48504e0ede8cd652b3ab227e1dc3aa21e4",
    show_call=TRUE) %>% filter(NAICS2017 %in%c(311710,424460,445220,424490)) %>%
    mutate(NESTAB=as.numeric(NESTAB)) %>%
    group_by(YEAR) %>%
    summarise(Establishments=sum(NESTAB,na.rm=TRUE),
              Confidential=sum(NESTAB_F=="D",na.rm=TRUE)) %>% 
...
write.csv(Total,file='F:/ESRs/ESR2025/Data/FINAL/Shoreside_Support_2025.csv')

#2022 doesn't come out until December 2024, but NAICS codes changed, so I programmed it up
# for (y in 2022) {
#   TEMP <- getCensus(
#     name = "nonemp",
#     vintage = y,
...
#     region = "state:10,24,36,34,42,37,51",
#     key="1cd45e48504e0ede8cd652b3ab227e1dc3aa21e4",
#     show_call=TRUE) %>% filter(NAICS2022 %in%c(424490,311710,424460,445250)) %>%
#     mutate(NESTAB=as.numeric(NESTAB)) %>%
#     group_by(YEAR) %>%
#     summarise(Establishments=sum(NESTAB,na.rm=TRUE),
#               Confidential=sum(NESTAB_F=="D",na.rm=TRUE)) %>% 
