...
save "F:\ESRs\ESR2026\Data\vesselsALL2026", replace


#delimit;
clear;
do "C:\Users\Geret.Depiper\Documents\Stata do\ORACLE_CONNECT";
cd "F:\ESRs\ESR2026\Data" ;
set mem 800m;
set more off; 
global firstyr =1996; 
global lastyr =2024; 
...
save "veslog_gear_2026.dta", replace;


set more off
cd "F:\ESRs\ESR2026\Data"
use "SOE_diversityrevenuedata_2026", clear
drop _merge*
merge m:1 permit tripid year using "veslog_gear_2026.dta", update
drop if _merge == 2
drop _merge
merge m:1 permit year using "F:\ESRs\ESR2026\Data\vesselsALL2026"
drop if _merge == 2
drop _merge
gen MAFMC = 0
...
preserve
	...
	tempfile mgear
	save "F:\ESRs\ESR2026\Data\majorgear", replace
restore
*************************************************************
merge m:1 year permit using "F:\ESRs\ESR2026\Data\majorgear"
	drop gearcode2
	...
	save "F:\ESRs\ESR2026\Data\FinalMAFMCNEFMC_2026", replace
	
cd "F:\ESRs\ESR2026\Data"
	use "F:\ESRs\ESR2026\Data\FinalMAFMCNEFMC_2026", clear
	rename real_revenue R_REV
	drop if year > $lastyr
preserve
	...
	line eShannonrev year, saving(ShannonEffMAFMCpermitrev_2026 , replace) graphregion(fcolor(white) icolor(white)) lwidth(medthick) ytitle("MAFMC" "Effective Shannon Index") 
	gen Region = "MA"
	gen Units = "effective Shannon"
	gen Var = "Permit revenue species diversity"
	rename (year eShannon) (Time Value) 
	export delimited Time Region Value Var Units using "F:\ESRs\ESR2026\Data\MAFMCspeciesdiversity_2026", replace	
restore
preserve
	...
	collapse (mean) Shannonrev eShannonrev HHIrev, by(year)
	line eShannonrev year, saving(ShannonEffNEFMCpermitrev_2026 , replace) graphregion(fcolor(white) icolor(white)) lwidth(medthick) ytitle("Effective Shannon Index") 
	gen Region = "NE"
	gen Units = "effective Shannon"
	gen Var = "Permit revenue species diversity"
	rename (year eShannon) (Time Value) 
	export delimited Time Region Value Var Units using "F:\ESRs\ESR2026\Data\NEFMCspeciesdiversity_2026", replace	
restore
preserve
	drop if MAFMCPermit == 0 | MAFMCPermit == .
	replace R_REV = 0 if R_REV == .
	...
	line fleet_count year , saving(FleetcountMAFMC_2026, replace) graphregion(fcolor(white) icolor(white)) lwidth(medthick) ytitle("Number of Fleets") 
	line Shannonrev year , saving(ShannonMAFMCfleetrev_2026 , replace) graphregion(fcolor(white) icolor(white)) lwidth(medthick) ytitle("Shannon Index") 
	...
	line eShannonrev year, saving(ShannonEffMAFMCfleetrev_2026 , replace) graphregion(fcolor(white) icolor(white)) ytitle("Effective Shannon Index") lwidth(medthick) legend(off)
	gen Region = "MA"
	gen Units = "effective Shannon"
	gen Var = "Fleet diversity in revenue"
	rename (year eShannonrev) (Time Value) 	
	export delimited Time Region Value Var Units using "F:\ESRs\ESR2026\Data\MAFMCfleetdiversity_2026", replace
	replace Units = "number of fleets"
	replace Var = "Fleet count"
	drop Value
	rename (fleet_count) (Value) 
	export delimited Time Region Value Var Units using "F:\ESRs\ESR2026\Data\MAFMCfleetcount_2026", replace
restore
preserve
	drop if NEFMCPermit == 0 | NEFMCPermit == .
	replace R_REV = 0 if R_REV == .
	...
	bysort year: egen HHIrev = sum(pHHIrev)
	bysort year: egen fleet_count = count(R_REV)
	export delimited using "F:\ESRs\ESR2026\Data\NEFMCfleetdiversity_2026", replace
	drop R_REV prev pshannonrev pHHIrev lencat majgear
	duplicates drop
	collapse (mean) Shannonrev eShannonrev HHIrev fleet_count, by(year)
	...
	gen Var = "Fleet diversity in revenue"
	rename (year eShannonrev) (Time Value) 
	export delimited Time Region Value Var Units using "F:\ESRs\ESR2026\Data\NEFMCavgfleetdiversity_2026", replace
	replace Units = "number of fleets"
	replace Var = "Fleet count"
	drop Value
	rename (fleet_count) (Value) 
	export delimited Time Region Value Var Units using "F:\ESRs\ESR2026\Data\NEFMCfleetcount_2026", replace
restore
